		...
		foreach($data as $td)
		{
			...
			$str .= "	<td class=\"ctr\">
							<div class=\"btn-group\">
								...
								<a href=\"" . base_URL() . 'instansi/kode_fak_kip/del/' . $td['id'] . "\" class=\"btn btn-warning btn-sm\" title=\"Hapus Data\" onclick=\"return confirm('Anda Yakin..?')\"><i class=\"fa fa-times\">  </i> Del</a>
							</div>
						</td>";
			$str .= "</tr>";
			$str .= $this->_print_recursive_table_kip($td['child'],$space);
			...
			foreach($row_fkip as $r){
				...
				$table .= "	<td class=\"ctr\">
								<div class=\"btn-group\">
									...
									<a href=\"" . base_URL() . 'instansi/kode_fak_kip/del/' . $r['id'] . "\" class=\"btn btn-warning btn-sm\" title=\"Hapus Data\" onclick=\"return confirm('Anda Yakin..?')\"><i class=\"fa fa-times\">  </i> Del</a>
								</div>
							</td>";
				$table .= "</tr>";
	...
	function kode_fak_kip(){
		...
		if ($mau_ke == "add-sub") {
			...
			if($this->form_validation->run() == TRUE){
			  ...
			  $this->session->set_flashdata("k", "<div class='alert alert-success'>Kode Fakultas KIP baru berhasil ditambahkan</div>");	
				
			  redirect('instansi/kode_fak_kip');
			...
			if($this->form_validation->run() == TRUE){
			  ...
			  $this->session->set_flashdata("k", "<div class='alert alert-success'>Kode Fakultas KIP berhasil diubah</div>");	
				...
				if(!$this->upload->do_upload('userfile')){
					...
					for ($i = 2; $i < ($xl_data->rowcount($sheet_index=0)); $i++){
						...
						*/
						...
						$username		= $xl_data->val($i, 6);
						$password		= md5($username);
						$tgl_aktif		= date('Y-m-d H:i:s');
						$status			= 'Y';
						$level			= $xl_data->val($i, 7);
						...
						$id_menu 		= $menu_yang_ok;
						$apps 			= $xl_data->val($i, 8);
						$email			= $xl_data->val($i, 9);
		...
		if ($mau_ke == "act_edt") {
			if (empty($idp)) {
				if ($this->upload->do_upload('logo')) {
					...
					$this->db->query("INSERT INTO instansi VALUES (NULL, '$nama', '$alamat', '".$up_data['file_name']."')");
				...
				if ($this->upload->do_upload('logo')) {
					...
					$this->db->query("UPDATE instansi SET nama = '$nama', alamat = '$alamat', logo = '".$up_data['file_name']."' WHERE id = '$idp'");
